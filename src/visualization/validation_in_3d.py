...
def render_cameras(batch: dict, resolution: int) -> Float[Tensor, "3 3 height width"]:
    ...
    num_target_views = batch["target"]["extrinsics"].shape[1] if "target" in batch else 0
    color = torch.ones(
    ...
    for k in ('extrinsics', 'intrinsics', 'near', 'far'):
        for c in ('context', 'target'):
            if c in batch:
                kwargs[k].append(batch[c][k][0])
        kwargs[k] = torch.cat(kwargs[k])
